    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW: #Вот нам написали
            if not sql.we_know_him(event.user_id): #Если его id ет в таблице person, то:
                ...
                button('Ну, погнали, жми кнопку', keyboard, event.user_id)
                ...
                try:
                    ...
                    if rel_foto:
                        ...
                        coll_i += 1
                        button('Жми кнопки и будет тебе счастье', full_kb, event.user_id)
                    else:
                        vk.messages.send(
                            random_id=random.randint(0, 2048),
                            peer_id=event.peer_id,
                ...
                button('Добавлено, продолжаем?', keyboard, event.user_id)
